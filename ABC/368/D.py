...
def main():
    ...
    flag_lst = [False] * (n + 1)
    for vertex in v:
        flag_lst[vertex] = True

    def dfs(node, parent):
        subtree_size = 0
        contains_required = flag_lst[node]

        for neighbor in graph[node]:
            if neighbor == parent:
                continue
            child_size, child_contains_required = dfs(neighbor, node)
            if child_contains_required:
        ...
        return subtree_size, contains_required
