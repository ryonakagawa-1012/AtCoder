...
def dfs(pos, graph_lst, visited, matrix, ans):
    visited[pos] = True
    for i in graph_lst[pos]:
        if not visited[i]:
            if matrix[i]:
                ...
                continue
            ans[0] += 1
            dfs(i, graph_lst, visited, matrix, ans)
...
def main():
    ...
    for H in range(h-1):
        for W in range(w-1):
            if s[H][W] == s[H][W+1] == ".":
                graph[H*w+W].append(H*w+W+1)
                graph[H*w+W+1].append(H*w+W)
            if s[H][W] == s[H+1][W] == ".":
                graph[H*w+W].append((H+1)*w+W)
                graph[(H+1)*w+W].append(H*w+W)
    ...
    for i in range(h*w):
        ...
        if not matrix[i]:
            # print(i)
            visited = [False] * (h*w)
            dfs(i, graph, visited, matrix, anst)
            ans.append(anst[0])
    # print(ans)
    mans = max(ans)
    if mans == 0 and not all(matrix):
        print(1)
    else:
        print(mans)
