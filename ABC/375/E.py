...
def main():
    ...
    diffs = [team_strength[i] - target_strength for i in range(3)]
    ...
    for i in range(3):
        for j in range(i + 1, 3):
            if diffs[i] > 0 and diffs[j] < 0:
                transfer = min(diffs[i], -diffs[j])
                diffs[i] -= transfer
                diffs[j] += transfer
                moves += transfer
            elif diffs[i] < 0 and diffs[j] > 0:
                transfer = min(-diffs[i], diffs[j])
                diffs[i] += transfer
                diffs[j] -= transfer
                moves += transfer
