...
def check(s, i, j):
    ...
    for ii in range(3):
        for jj in range(3):
            if s[i+ii][j+jj] == "." or s[i+ii+6][j+jj+6] == ".":
                cond1 = False
    if all([s[i+3][j + k] == "." for k in range(5)]) and all([s[i+5][j + 5 + k] == "." for k in range(5)]) and \
        all([s[i+k][j+3] == "." for k in range(5)]) and all([s[i+5+k][j + 5] == "." for k in range(5)]):
        cond2 = True
