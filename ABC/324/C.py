...
def create_pattern(t: str) -> re.Pattern:
    # 2. T のいずれか1つの位置に英小文字を1つ挿入
    insert_pattern = '|'.join([re.escape(t[:i]) + '[a-z]' + re.escape(t[i:]) for i in range(len(t) + 1)])
    
    # 3. T からある1文字を削除
    delete_pattern = '|'.join([re.escape(t[:i]) + re.escape(t[i+1:]) for i in range(len(t))])
    
    # 4. T のある1文字を別の英小文字に変更
    change_pattern = '|'.join([re.escape(t[:i]) + '[a-z]' + re.escape(t[i+1:]) for i in range(len(t))])
    
    # 全てのパターンを結合
    full_pattern = f"({insert_pattern}|{delete_pattern}|{change_pattern})"
    
    return re.compile(full_pattern)

# 使用例
t = "example"
pattern = create_pattern(t)
