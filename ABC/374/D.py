...
def main():
    ...
    perm = list(permutations(abcd, n))
    zenngo = list(list(map(int, bin(i)[2:].zfill(n+1))) for i in range(2**(n+1)))
    # print(perm)
    # print(zenngo)
    ans = float("inf")
    for AtoD in perm:
        ...
        for i in range(n):
            a, b, c, d = AtoD[i]
            print(a, b, c, d)
            for binlst in zenngo:
                print(binlst)
                innsatu = 0
                for j in range(n+1):
                    if binlst[j] == 0:
                        innsatu += s*euclidean_distance((pri_x, pri_y), (a, b))
                        innsatu += t*euclidean_distance((a, b), (c, d))
                        pri_x += a
                        pri_y += b
                    else:
                        innsatu += s*euclidean_distance((pri_x, pri_y), (c, d))
                        innsatu += t*euclidean_distance((c, d), (a, b))
                        pri_x += c
                        pri_y += d
                ans = min(ans, innsatu)
    print(ans)
