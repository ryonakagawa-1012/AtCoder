...
def main():
    ...
    masu = [[True for _ in range(w)] for _ in range(h)]
    uf = UnionFind(h * w)
    ans = h * w
    ...
    for _ in range(q):
        ...
        if masu[r][c]:
            ...
            for dx, dy in directions:
                nr, nc = r + dx, c + dy
                if is_valid(nr, nc) and not masu[nr][nc]:
                    uf.union(index(r, c), index(nr, nc))
        else:
            found = False
            for dx, dy in directions:
                nr, nc = r, c
                while is_valid(nr + dx, nc + dy):
                    nr += dx
                    nc += dy
                    if masu[nr][nc]:
                        masu[nr][nc] = False
                        ...
                        for dx2, dy2 in directions:
                            ...
                            if is_valid(nr2, nc2) and not masu[nr2][nc2]:
                                uf.union(index(nr, nc), index(nr2, nc2))
                        found = True
                        break
                if found:
                    break
