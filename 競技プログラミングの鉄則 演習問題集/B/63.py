...
def main():
    ...
    sy, sx = sep_read(int)
    gy, gx = sep_read(int)
    maze = [readline() for i in range(r)]

    # print_2d(maze)

    graph_lst = defaultdict(list)

    for i in range(r):
        for j in range(c):
            pos = i*c+(j+1)
            if j != c and maze[i][j] == "." and maze[i][j+1] == ".":
                nex = pos+1
                graph_lst[pos].append(nex)
                graph_lst[nex].append(pos)
            if i != r and maze[i][j] == "." and maze[i+1][j] == ".":
                nex = pos+c
                graph_lst[pos].append(nex)
                graph_lst[nex].append(pos)
